...
// Initialize tables in Postgres if connected
async function initPgTables() {
  ...
  try {
    ...
    console.log("ℹ️ PostgreSQL local instance not reachable. Using file database mode.");
    usePostgres = false;
    return false;
  }
}

// Self-executing DB initialization
initPgTables().catch(() => {});
...
export const db = {
  async findUserByEmail(email) {
    ...
    return ensureFileDb().users.find((u) => u.email.toLowerCase() === lowerEmail) || null;
  },
  ...
  async findUserById(id) {
    ...
    return ensureFileDb().users.find((u) => u.id === id) || null;
  },
  ...
  async insertMemory({ user_id, text, embedding = [], importance_score = 0, pinned = false }) {
